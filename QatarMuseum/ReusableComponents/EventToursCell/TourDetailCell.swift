...
class TourDetailCell: UITableViewCell {
    ...
    func setParksCellValues(eventTourList: EventToursList,currentRow:Int?) {
        
        
        titleLabel.text = eventTourList.title?.uppercased()
        let parkDesc = eventTourList.description?.replacingOccurrences(of: "<[^>]+>|&nbsp;|&|#039;", with: "", options: .regularExpression, range: nil)
        ...
        if let imageUrl = eventTourList.image{
